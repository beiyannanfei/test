	...
	async.auto({
		checkUnique: function (cb) {        //检测订单是否已经存在
			findOrderByuniqueId(uniqueId, function (err, data) {
				if (!!err) {
					console.log("[%j] unionMallOrderConsumer checkUnique err: %j, order: %j, offset: %j",
						new Date().toLocaleString(), err, order, orderStr.offset);
					return cb(err);
				}
				if (data) { //订单已经存在
					console.log("[%j] unionMallOrderConsumer checkUnique data exists, order: %j, uniqueId: %j, offset: %j",
						new Date().toLocaleString(), order, uniqueId, orderStr.offset);
					return cb(uniqueId);
				}
		...
		minusUserCv: ["checkUnique", function (cb) {        //扣除用户余额
		...
		return console.log("[%j] ================= save unionMallOrderConsumer success, uniqueId: %j, offset: %j, results: %j",
			new Date().toLocaleString(), uniqueId, orderStr.offset, results);
	});
}
...
function sendWxRed(order, cb) {     //发送微信红包
	...
	mWxhb.sendHb(1, order._id.toString(), order.yyyapp_id, order.yyy_openid, +order.redInfo.firstSpend, order.open_id, act_name, remark, function (err, doc) {
		...
		mUnionMallRed.sendRedInfo2GG(order.uniqueId, +order.redInfo.firstSpend);
		mUnionMallRed.sendRedInfo2yj(order, order.redInfo);
		return cb(null, doc)
	});
}
...
function updateOrder(order, offset) {       //订单更新
	...
	async.auto({
		findOrder: function (cb) {      //获取已存order
			findOrderByuniqueId(uniqueId, function (err, data) {
				if (!!err) {
					console.log("[%j] unionMallOrderConsumer-findOrderByuniqueId err: %j, order: %j, offset: %j",
		...
		sendRed: ["minusUserCv", function (cb) {
			if (unionOrder.redState == typeConfig.unionMallOrderState.success || unionOrder.redState == typeConfig.unionMallOrderState.partSuccess) {
				return cb(null, "already send");
			}
			if (order.status != mUnionOrderPaySate.pay) {
				return cb(null, "not pay");
			}
			var redInfo = unionOrder.redInfo || order.redInfo;
			if (!redInfo) {
				return cb(null, "no redInfo");
			}
			var sendMoneyNum = redInfo.firstSpend;
			unionOrder.redInfo ? "" : (unionOrder.redInfo = redInfo);
			sendWxRed(unionOrder, function (err, data) {
				if (!!err) {
					order.redState = typeConfig.unionMallOrderState.sendRedError;
					return cb(null, "send red err: " + err);
				}
				order.redState = typeConfig.unionMallOrderState.success;
				if (sendMoneyNum < unionOrder.expect_cash) {
					order.redState = typeConfig.unionMallOrderState.partSuccess;
				}
				return cb(null, "success");
			});
		}],
		updateOrder: ["sendRed", "revertCv", function (cb) {      //更新订单
			...
			if (unionOrder.redState != typeConfig.unionMallOrderState.success && null == order.redState) {    //如果红包未发送成功更改红包状态
				order.redState = typeConfig.unionMallOrderState.wait;   //红包状态
...
function dealRefundOrder(order) {
	...
	if (order.redState != typeConfig.unionMallOrderState.success && order.redState != typeConfig.unionMallOrderState.partSuccess) {     //未发红包不处理
		return;
	}
	...
	var value = order.redInfo.firstSpend || order.redInfo.spend || 0;
	redisClient.HINCRBY(key, field, value, function (err, o) {
		console.log("[%j] dealRefundOrder err: %j, o: %j, field: %j, value: %j, id: %j", new Date().toLocaleString(), err, o, field, value, order._id.toString());
	});
}
...
function revertUserCv(order, updateDoc, cb) {   //当用户退款或者取消订单且余额已扣恢复用户余额
	...
	if (order.redState == typeConfig.unionMallOrderState.success || order.redState == typeConfig.unionMallOrderState.partSuccess) {     //红包已发
		return cb ? cb(null, null) : "";
	}
	...
	var spendUserCv = redInfo.firstSpend || redInfo.spend;    //扣除的用户余额
