...
for (let index = 0; index < module.children.length; ++index) {
	let item = module.children[index];
	var modulePath = item.id;
	if (modulePath.indexOf("node_modules/joi") != -1) {
		joiPath = modulePath;
		break;
	}
}
// console.log(joiPath);
let langPath = path.join(joiPath, "../language.js");
...
for (var i in language_cn) {
	var index = i;
	var lanObj = language_cn[i];
	_.extend(langModule.errors[index], lanObj);
}

// console.log(langModule.errors.number.base);
// _.extend(langModule.errors.number, {base: "必须是个数字aaaaaaa"});
// _.extend(langModule.errors.array, {min: "必须至少包含 {{limit}} 个元素"});
