...
const client = JPush.buildClient("b36d0cf1302752077b1a8d8e", "fe5999503265e6ec8a1fa5a7", 5);


client
	.push()
	.setPlatform(JPush.ALL) //设置 platform，本方法接收 JPush.ALL, android, ios这几个参数
	.setAudience(JPush.tag_not("disable_record_push_001")) //设置 audience，本方法接收 JPush.ALL，或者是 tag(), tag_and(), alias(), registration_id() 创建的对象
	// .setAudience(JPush.ALL) //设置 audience，本方法接收 JPush.ALL，或者是 tag(), tag_and(), alias(), registration_id() 创建的对象
	.setNotification(
		'Hi, JPush', //设置 notification，本方法接收 ios(), android(), winphone()等方法创建的对象，如果第一个参数为字符串，则指定全局的 alert
		...
		// 	'android alert', // string 通知内容。必传
		// 	"title",  //string 通知标题。
		// 	"builder_id", //int Android SDK 可设置通知栏样式，这里根据样式 ID 来指定该使用哪套样式。
		// 	"extras", // object 自定义 key / value 信息，以供业务使用。
