...
var db = models.getDb();
var GridStore = models.getGridStore();
    ...
    if (config.NODE_ENV == 'dev'){
        ...
        gs.open(function(err, girdfs){
            ...
            girdfs.writeFile(file.path, function(err, result){
                ...
                var url = config.domain + config.path + "/i/" + id;  // /i for image
                console.log(url)
                fn(null, url);
            })
        })
        ...
        childProcess.exec(cmdArr.join(' '), function(err){
            if (err){
                ...
                if (config.NODE_ENV == 'ali'){
                    var url = 'http://a.cdn.mtq.tvm.cn' + config.nginxFileDir + '/' + fileName;
                } else{
