...
exports.getStoreShareParam = function(req, res){
    ...
    var param = {
        ...
        title: store.share?store.share.title:store.name,
        desc: store.share?store.share.desc:store.name
    }
...
exports.getVideoShareParam = function(req, res){
    ...
    var generateShareParam = function(doc){
        var shareParam = {
            link: tkConfig.getAuthDomain(token) + "/oauth?wx_token=" + token + "&token=7fda67277f&redirect_uri=" + encodeURIComponent(config.domain + '/pointMall/auth/goods/' + goods._id + '?wx_token=' + token + '&demandId=' + demandId)
        }
    ...
    if (demandId){
        ...
        Goods.findById(demandId, function(err, o){
            generateShareParam({title: o.name, desc: o.ext.videoRes.desc, imgUrl: o.pic})
        })
    } else if (goods.type == mGoods.type.demandPackage && goods.ext.package.length > 0){
...
exports.getGoodsShareParam = function (req, res) {
    ...
    var param = {
        link: link,
        imgUrl: goods.ext.share ? goods.ext.share.img_url : goods.pic,
        title: goods.ext.share ? goods.ext.share.title : goods.name,
