...
dbUtils.prototype = {
    ...
    insert: function (docs, callback) {
        ...
        thisthisDb[this.collection].insert(docs, callback)
        redisCache.delMulti(this.collection);
    },
    save: function(doc, callback) {
        var thisDb = this.curDb
        thisDb[this.collection].save(doc, function(err){
            callback(err, doc)
            if (!err && doc._id) {
                redisCache.set(doc._id.toString(), JSON.stringify(doc));
            }
        })
        redisCache.delMulti(this.collection);
    },
        ...
        redisCache.get(redisKey, function (err, o) {
            if (err) {
                thisDb[collection].findById(_id, options, function (err, db_result) {
                    ...
                    if (collection == 'goods' || collection == 'coupon') {
    ...
    distinct: function(spec, field, callback) {
        ...
        thisDb[collection].distinct(spec, field, function (err, db_result) {
            callback(err, db_result);
        });
    }
};
