...
var Behavior = new dbUtils('behaviors');
...
var addGroup = function (wxToken, activity, title, result, callback) {
    ...
    var addGroupsResult = function(_id,result){
        groupsCollection.updateById(_id,{$addToSet:{results:result}},function(err,doc){
            if(err){
                console.log(err);
            }
        });
...
//            console.time('Groups.findOne');
            groupsCollection.findOne({wxToken: wxToken, activity: activity, key: title}, {_id: 1}, function (err, doc) {
                if (err) {
                    console.log(err);
                }
                if (doc) {
...
//                    var group = new Groups(obj);
                    ...
                    groupsCollection.save(obj, function (err, group) {
                        if (err) {
                            console.log(err);
                        }
//                        console.timeEnd('Groups.findOne');
...
var addBehavior = function (obj,openId) {
    ...
    async.eachSeries(openIds, function (_openId, done) {
        ...
        obj.openId = _openId;
        userCollection.findById(_id, {nickName: 1, headImg: 1}, function (err, doc) {
            if (err) {
                console.log(err);
            }
            var nickName = '';
...
//            var behavior = new Behavior(obj);
            Behavior.save(obj, function (err, doc) {
                if (err) {
                    console.log(err);
                }
            });
...
//            console.time('addGroupUserIds One redis');
            ...
            groupUserIdsCollection.findOne({groupId: groupId, openId: openId}, function (err, doc) {
                if (err) {
                    console.log(err);
                }
                if (!doc) {
                    ...
                    groupUserIdsCollection.save(obj, function (err, data) {
                        if (err) {
                            console.log(err);
                        }
                        if (data) {
