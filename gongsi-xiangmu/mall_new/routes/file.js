...
exports.uploadFile = function(file, fn){
    if (config.NODE_ENV == 'dev'){
        httpUtils.postFile('http://tmall.mtq.tvm.cn/image/upload', 'file', file.path, function(err, results){
            if (!!err) {
                return fn(err);
            }
            if (!results || !results.url) {
                return fn("url is null");
            }
            return fn(null, results.url);
        });
    } else {
...
exports.getFile = function(req, res){
    ...
    var gs = new dbUtils.GridStore(dbUtils.db, new dbUtils.ObjectID(id), 'r', {safe: true});
...
exports.postFile = function(req, res){
    ...
    if(file.size > 1000 * 1024){
        return res.send({error: 1, msg: '文件太大，不能超过1M'});
    }
