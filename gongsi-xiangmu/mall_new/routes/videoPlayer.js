...
var playerhistorieCollection = new dbUtils('playerhistories');
...
exports.getGoodsVideoRes = function(req, res){
    ...
    var loadVideoGoods = function(){
        ...
        goodsCollection.find({_id: {$in: ids}}, {'ext.videoRes': 1, 'ext.detail': 1, name: 1, pic: 1, price: 1}, function(err, docs){
            if (err){
                final([])
            } else {
                final(docs)
    ...
    if (goods.type == typeConfig.goods.type.demandPackage){
        loadVideoGoods()
    } else {
        final([{
...
exports.addPlayerHistory = function(req, res){
    ...
    playerhistorieCollection.update(SPEC, UPDATE_SPEC, {upsert: true, multi: true}, function(err, doc){
        if (err){
            console.log(err);
            return res.send(500)
        } else {
...
exports.delPlayerHistory = function(req, res){
    ...
    playerhistorieCollection.update(SPEC, UPDATE_SPEC, function(err, doc){
        if (err){
            console.log(err);
            return res.send(500)
        } else {
...
exports.playerHistory = function(req, res){
    ...
    var findHistory = function(){
        playerhistorieCollection.findOne(condition, function(err, doc){
            if (err){
                final(null)
            } else {
                findGoods(doc);
    ...
    var findGoods = function(history){
        ...
        goodsCollection.find({_id: {$in: ids}}, {'ext.videoRes': 1, name: 1, pic: 1}, function(err, docs){
            if (err){
                final([])
            } else {
...
exports.videoSign = function(req, res){
    ...
    var option = {}
    option[uKey] = 1
    playerhistorieCollection.findOne(SPEC, option, function(err, doc) {
        var data = {
            sign: sign
        }
