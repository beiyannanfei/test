...
exports.addAddress = function(req, res){
    ...
    var findAddressCount = function(o){
        addressCollection.count({openId: openId, deleted: {$ne: 1}}, function(err, count){
            if (err){
                console.log(err);
    ...
    var setDefault = function(address){
        address.isDefault = 1
        addressCollection.updateById(address._id, {$set: {isDefault: 1}}, function(err, o){
            if (err){
                console.log(err);
                return res.send(500, err)
            } else{
...
exports.saveAddress = function(openId, token, addInfo, callback){
    ...
    addressCollection.findOne(doc, function(err, add){
        if (err){
            console.log(doc);
            ...
            addressCollection.save(doc, function(err, o){
                if (err){
                    callback(err);
                } else if (!o){
...
exports.setDefault = function(req, res){
    ...
    var resetDefault = function(){
        addressCollection.update({openId: openId, isDefault: 1}, {$set: {isDefault: 0}}, {multi:true}, function(err){
            if (err){
                return res.send(500, err)
            } else{
                setDefault();
    ...
    var setDefault = function(){
        addressCollection.updateById(addressId, {$set: {isDefault: 1}}, function(err){
            if (err){
                return res.send(500, err)
            } else{
                return res.send(200)
...
exports.findAddressById = function(id, callback){
    addressCollection.findById(id, callback);
}
...
exports.getAddressByIds = function(addressIds, callback){
    ...
    addressCollection.find(condition, callback);
}
...
exports.midAddressLoader = function(req, res, next){
    ...
    addressCollection.findById(id, function(err, doc){
        if (err){
            return res.send(500, err);
        }
        if (!doc){
...
exports.updateAddress = function(req, res){
    ...
    addressCollection.updateById(address._id, UPDATE_SPEC, function(err){
        if (err){
            return res.send(500, err);
        } else{
            return res.send(200);
...
exports.deleteAddress = function(req, res){
    ...
    addressCollection.updateById(address._id, {$set: {deleted: 1}}, function(err, doc){
        if (err){
            return res.send(500, err);
        } else{
            return res.send(200);
...
exports.reselect = function(req,res){
    ...
    addressCollection.update({openId: openId}, {$pull:{ids: orderId}},{multi:true}, function(err,doc){
        if (err){
            return res.send(500, err)
        } else{
            addressCollection.updateById(addressId, {$addToSet:{ids:orderId}}, function(err, doc){
                if (err){
                    return res.send(500, err);
                } else{
                    return res.send(200);
...
exports.getAddress = function(req,res) {
    ...
    addressCollection.findOne({token:token,openId:openId,ids:orderId},function(err,doc){
        if (err){
            return res.send(500, err);
        } else{
            if(doc && doc.addInfo){
...
exports.list = function(req,res) {
    ...
    addressCollection.find({token:token,ids:orderId},function(err,docs){
        if (err){
            return res.send(500, err);
        } else{
            if(docs){
                ...
                findUserByIds(ids,function(users){
                    _.each(docs, function (doc) {
                        ...
                        if (users && users[doc.openId]) {
                            ...
                            _id=  users[doc.openId]._id.toString();
                        }else{
...
var findUserByIds = function (openIds, callback) {
    ...
    userCollection.find(condition,{_id:1,nickName:1,headImg:1,openId:1}, function (err, docs) {
        if (err) {
            console.log(err);
            return callback({});
        }
...
exports.queryDefault = function(req, res){
    addressCollection.findOne({token: req.token, openId: req.openId, isDefault: 1, deleted: {$ne: 1}}, function(err,doc){
        if (err){
            return res.send(500, err);
        } else{
            if (!doc){
...
exports.fillAddress = function(req, res, next){
    ...
    if (goods.type == typeConfig.goods.type.goods){
        if (req.body.address){
            exports.saveAddress(req.openId, req.token, req.body.address, function(err, addInfo){
                if (err){
                    res.send(500, err)
                } else {
