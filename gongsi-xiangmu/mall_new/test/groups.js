...
var saveGroup = function (){
    ...
    groupsCollection.save({key: groupName, wxToken: wxToken, createTime: new Date()}, function (err, doc) {
        if (err) {
            console.log(err);
            callback(null);
        } else {
        ...
        if (data && data.status === 'success') {
            getUserByRedis(redisKey, function (reply) {
                if (reply && reply.status === 'success') {
                    ...
                    groupsCollection.updateById(doc._id,{$set:{openIds:reply.reply,parameters:parameters}},function (err, updateDoc) {
                        if (err) {
                            console.log(err);
                        }else{
                            if (updateDoc) {
...
var sava = function (){
    ...
    getUserByRedis(redisKey, function (reply) {
        if (reply && reply.status === 'success') {
            ...
            var parameters = JSON.stringify(condition);
            groupsCollection.updateById('54229bdd6ba0158f4e000004',{$set:{openIds:reply.reply,parameters:parameters}},function (err, updateDoc) {
                if (err) {
                    console.log(err);
                }else{
                    if (updateDoc) {
