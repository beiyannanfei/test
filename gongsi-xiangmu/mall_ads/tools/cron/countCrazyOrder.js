...
function countOrder() {
    queueClient.BRPOP("crazyLotteryEnd", 0, function (err, data) {
        var doc = {};
        console.log("***countCrazyLottery arguments: %j", arguments);
        if (!!err) {
            console.error("redis pop err: %j", err);
            return setTimeout(countOrder, 30000);   //查询redis出错后30s恢复
        }
        if (data && _.isArray(data) && data.length > 1) {
            var keys = '' + JSON.parse(data[1]).lotteryId;
            var newkeys = "pushCrazyLottery" + keys;
        }
        if (!keys) {
            return countOrder()
        }
        doc = _.extend(doc, {lottery_id: keys});
        doc = _.extend(doc, {lottery_type: "crazy"});
        crazylotteryCollection.findById(keys, function (err, lotteryDoc) {  //从数据库中获取tvmid
            var tvmId = "";
            if (!!err) {
...
//            url = "http://ads.dev.tvm.cn/Api/getTimeInfo/lottery_id/55a0f25998570dcf2f464192"; keys = "55a0f25998570dcf2f464192";
            ...
            httpUtils.httpGet(url, function (err, response) {   //获取开始日期，开始时间，结束时间
                ...
                queueClient.hlen(keys + 'allUsers', function (err, count) {
                    doc.totalPeople = +count;
                    queueClient.lrange(newkeys, 0, -1, function (err, datas) {
                        console.log(datas);
                        if (!!err) {
                        ...
                        _.each(datas, function (val) {
                            ...
                            totalMoney += +money;
                            var userName = val.user.name ? val.user.name : "";
                            var icon = val.user.icon ? val.user.icon : "";
                            var openId = val.user.openId ? val.user.openId : "";
                            var prizeId = "";
                            if (temp.id) {
                                prizeId = temp.id;
                            }
                            ...
                            if (type == typeConfig.prizeType.wxred) {   //微信红包
                                ...
                                if (!other[type]) {
                                    other[type] = {name: name, num: 0, money: 0, type: type, rate: rate, prizeId: prizeId, users: []};
                                }
                                other[type].users.push({name: userName, icon: icon, openId: openId});
                                other[type].num = other[type].num + 1;
                            }
                        });
                        var prize = [];
