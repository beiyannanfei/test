...
!function (a, b) {
    ...
    g.anim = ["layui-anim-01", "layui-anim-02", "layui-anim-03", "layui-anim-04", "layui-anim-05", "layui-anim-06", "layui-anim-07"], f.pt.config = {
        ...
        var c = this, d = c.index, f = c.config, h = f.zIndex + d, i = "object" == typeof f.title, j = !(!f.maxmin || 1 !== f.type && 2 !== f.type || /^\d+%$/.test(f.area[0]) && /^\d+%$/.test(f.area[1])), k = f.title ? '<div class="layui-layer-title" style="' + (i ? f.title[1] : "") + '">' + (i ? f.title[0] : f.title) + "</div>" : "";
