/*
 * Comparison Test between Boris and Hiboris parsing .
 */
...
b.on( 'match', function ( e, d, convert ) {
    bresult = convert( d );
    log( '- Hiboris match data:', e ? ' (Redis error)' : '' );
    log( util.inspect( bresult, false, 3, true ) );
    // now parse with Hiboris (hiredis)
    hb.parse( rdata );
} );

hb.on( 'match', function ( e, d, convert ) {
    hbresult = convert( d );
    log( '- Boris match data:', e ? ' (Redis error)' : '' );
    log( util.inspect( hbresult, false, 3, true ) );
    log( '- check if parsed results are the same.' );
    assert.deepEqual( bresult, hbresult );
} );
